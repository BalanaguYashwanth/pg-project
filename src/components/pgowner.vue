<template>
  <div  id="mainpage">
    ...
    <div class="container">
      <form>
        <div class="card mx-auto mb-4 shadow-sm  bg-white rounded " style="width: 50rem">
          <div class="card-body">
            <textarea
              style="border: none"
              rows="3"
        ...
        <div v-for="(all, index) in postdata()" v-bind:key="index">
...
export default {
  ...
  methods: {
    ...
    postdata:function(){
      ...
      {
        if(posts[data].pgname == this.$store.state.pgname  )
        {
    ...
    fileurl: function (text, pgname, username) {
      ...
      uploadedTask.on(
        ...
        axios.post('http://127.0.0.1:5000/post/posts',{
          ...
          console.log(res)
          this.textdata=""
          this.$store.dispatch('postsaction')
        })
  ...
  async created() {   
    ...
    await this.$store.dispatch("postsaction")
