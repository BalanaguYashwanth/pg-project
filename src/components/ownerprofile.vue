  ...
  methods: {
    ...
    file:function(event){
      ...
      getcurrentuser:function(){
        ...
        let allusers=this.$store.state.mainuserprofile
        console.log('allusers',this.$store.state.mainuserprofile)
        for(let key in allusers)
        {
          let alluser=allusers[key]
          for(let key1 in alluser)
          {
            if( alluser[key1].userid == localStorage.getItem('uid') )
            {
              this.phonenumber1=alluser[key1].phonenumber
              this.pgname1=alluser[key1].pg_name
              this.gender1=alluser[key1].gender
              this.age1=alluser[key1].age
              this.pk_id=alluser[key1].id
            }
          }
        }
      }
